...
const COLORS = [
  ...
  { text: "text-red-500", border: "border-red-100", bg: "bg-red-100", imgBorder: "border-red-200" },
  { text: "text-yellow-700", border: "border-yellow-100", bg: "bg-yellow-100", imgBorder: "border-yellow-300" },
];
...
const ResourceCardItem = React.memo(
  ({ card, index, hovered, setHovered }: { 
    ...
    if (isExternal) return <a href={card.href} target="_blank" rel="noreferrer" className="block h-full">{content}</a>;
    return <Link href={card.href} className="block h-full">{content}</Link>;
  }
);
