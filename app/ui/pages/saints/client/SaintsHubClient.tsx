...
type SaintsHubClientProps = {
  cards: Omit<HubCardProps, "color">[];
};

export function SaintsHubClient({ cards }: SaintsHubClientProps) {
    ...
    <>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-black/40 font-inria text-base">
          {filter ? `No saints found matching "${filter}".` : "No saints available for this grade yet. Check back soon!"}
        </div>
      ) : (
